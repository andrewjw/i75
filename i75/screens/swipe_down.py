...
class SwipeDown(ScreenWipe):
    def __init__(self,
        ...
        self.pixel_per_ms = self.height / float(self.duration)
    ...
    def update(self,
        ...
        if old_divide != new_divide:
            for y in range(old_divide, new_divide):
    ...
    def __get_divide(self, time: int) -> int:
        return min(round(time * self.pixel_per_ms), self.height)
