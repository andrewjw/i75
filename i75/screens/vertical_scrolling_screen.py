...
class VerticalScrollingScreen(Screen):
    def __init__(self,
                 offset_x: int,
                 offset_y: int,
                 viewport_width: int,
    ...
    def get_pixel(self, x: int, y: int) -> Colour:
        ...
        return self.screen.get_pixel(x - self._offset_x,
                                     y - self._offset_y + self._scroll_y)
    ...
    def update(self,
        ...
        for x in range(self.viewport_width):
            for y in range(self.viewport_height):
                c1 = self.screen.get_pixel(x, y + old_scroll)
                c2 = self.screen.get_pixel(x, y + self._scroll_y)
                if c1 != c2:
